...
# Usage
The following is an example that relies on simulated data and makes use of the `ggplot2` and `reshape` R packages.  Since there is no data to load for this example, it should be relatively easy to run and to inspect the variables to understand the desired format.  This should also help make clear the underlying assumptions of the model.
...
guideLevelStats$screen="test" # here, we're only doing the one screen - this simulation
